...
public final class SimpleUpdater {
  ...
  public void checkForUpdates(final ResponseHandler responseHandler) {
    Thread updaterThread = new Thread(() -> {
      ...
      try {
        ...
        String updateName = (String) ((JSONObject) filesArray.get(filesArray.size() - 1))
            .get("name");
        ...
        plugin.getLogger().warning(
            "Could not check for updates because of a version format error: " + e.getMessage()
                + ".");
  ...
  private static class PluginVersion {
    ...
    /**
     * Compares this version with another version, using the array "versionNumbers". Examples: v1.12
     * is newer than v1.2 ([1, 12] is newer than [1, 2]) v2.01 is equal to v2.1 ([2, 1] is equal to
     * [2, 1])
     *
