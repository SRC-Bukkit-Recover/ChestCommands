...
public class EnchantmentSerializer {
  ...
  static {
    ...
    enchantmentsMap
        .put(formatLowercase("Projectile Protection"), Enchantment.PROTECTION_PROJECTILE);
  ...
  public static Map<Enchantment, Integer> loadEnchantments(String input, String iconName,
    ...
    for (String singleEnchant : input.split(";")) {
      ...
      if (singleEnchant.contains(",")) {
        ...
        try {
          ...
          errorLogger.addError("The icon \"" + iconName + "\" in the menu \"" + menuFileName
              + "\" has an invalid enchantment level: " + levelSplit[1]);
        }
      ...
      if (ench == null) {
        errorLogger.addError("The icon \"" + iconName + "\" in the menu \"" + menuFileName
            + "\" has an invalid enchantment: " + singleEnchant);
      } else {
