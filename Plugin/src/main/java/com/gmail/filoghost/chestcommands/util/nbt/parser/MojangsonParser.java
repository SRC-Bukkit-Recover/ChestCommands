...
import com.gmail.filoghost.chestcommands.util.nbt.NBTByte;
import com.gmail.filoghost.chestcommands.util.nbt.NBTByteArray;
import com.gmail.filoghost.chestcommands.util.nbt.NBTCompound;
import com.gmail.filoghost.chestcommands.util.nbt.NBTDouble;
import com.gmail.filoghost.chestcommands.util.nbt.NBTFloat;
import com.gmail.filoghost.chestcommands.util.nbt.NBTInt;
import com.gmail.filoghost.chestcommands.util.nbt.NBTIntArray;
import com.gmail.filoghost.chestcommands.util.nbt.NBTList;
import com.gmail.filoghost.chestcommands.util.nbt.NBTLong;
import com.gmail.filoghost.chestcommands.util.nbt.NBTLongArray;
import com.gmail.filoghost.chestcommands.util.nbt.NBTShort;
import com.gmail.filoghost.chestcommands.util.nbt.NBTString;
import com.gmail.filoghost.chestcommands.util.nbt.NBTTag;
import com.gmail.filoghost.chestcommands.util.nbt.NBTType;
...
public final class MojangsonParser {
    ...
    while (currentChar() != ']') {
      ...
      if (listType == null) {
        ...
        throw parseException(
            "Unable to insert " + elementType + " into ListTag of type " + listType);
      }
      list.add(element);
      if (!advanceToNextArrayElement()) {
        break;
  ...
  /**
   * Verifies whether the current character is of given value and whether the parser can advance. If
   * these conditions are met, the parser advances by one. If these conditions are not met, an
   * exception is thrown.
   *
   * @param c the expected character
   * @throws MojangsonParseException if {@link #currentChar()} does not equal {@code c} or if {@link
  ...
  private void expectChar(char c) throws MojangsonParseException {
    ...
    throw new MojangsonParseException(
        "Expected '" + c + "' but got '" + (hasNext ? Character.valueOf(currentChar())
            : "<End of string>") + "'", this.str, this.index + 1);
  }

  /**
   * Verifies that the string has ended or that all characters from the next character on only
